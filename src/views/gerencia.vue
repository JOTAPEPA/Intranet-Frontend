            ...
            <div class="main-content">
                ...
                <div class="content-header">
                    <div class="header-left">
                        <h4 class="page-title">Gerencia</h4>
                        <p class="page-subtitle">Gestiona Documentos de Gerencia</p>
                    </div>
                            ...
                            <q-tr :props="props" class="table-row">
                                ...
                                >
                                    ...
                                    <div v-else-if="col.name === 'cantidadArchivos'" class="files-count-cell">
                                        <q-chip 
                                            :color="props.row.tieneArchivos ? 'blue' : 'grey'" 
                                            text-color="white" 
                                            :icon="props.row.tieneArchivos ? 'attach_file' : 'block'"
                                            size="sm"
                                        >
...
async function searchDocuments() {
    ...
    try {
        ...
        const allDocuments = await getData('/gerencia');
...
async function downloadDocuments(doc) {
    ...
    try {
        ...
        showNotification('positive', 'Descarga iniciada', `Se están descargando ${doc.archivos.length} archivo(s)`);
...
async function deleteDocument(document) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3001/api/gerencia/${document._id}`, {
            method: 'DELETE',
            headers: {
...
async function getDocuments() {
    try {
        loading.value = true;
        console.log('📥 Cargando documentos de gerencia...');
        
        const response = await getData('/gerencia');
