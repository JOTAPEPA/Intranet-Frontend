                            ...
                            <q-tr :props="props" class="table-row">
                                <q-td v-for="col in props.cols" :key="col.name" :props="props" class="table-cell">
                                    ...
                                    <div v-else-if="col.name === 'cantidadArchivos'" class="files-count-cell">
                                        <q-chip :color="props.row.tieneArchivos ? 'blue-1' : 'grey-3'"
                                            :text-color="props.row.tieneArchivos ? 'blue-8' : 'grey-6'"
                                            :icon="props.row.tieneArchivos ? 'attach_file' : 'attach_file'" size="sm">
                                            {{ col.value }}
                                        </q-chip>
                                    </div>
...
async function deleteDocument(document) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3001/api/talento-humano/${document._id}`, {
            method: 'DELETE',
            headers: {
...
async function getDocuments() {
    try {
        loading.value = true;
        console.log('📥 Cargando documentos de talento humano...');

        const response = await getData('/talento-humano');
